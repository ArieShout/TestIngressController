...
for pod in 400 380 340 300 280 240 200 160 120 80 40 20 10; do
    ...
    cat ips | xargs -i ./pass-cp del {}
    cat ips | xargs -i ./run.cmd.sh {}
    cat ips | xargs -i ./pass-cp sync {}
    cat ips | xargs -i ./run.cmd.sh {}
    ...
    for ((i=0;i<4;i++)); do
        ...
        #./2020-06-09-main.pod.svc.sh ${dir}
        #sleep 30
        #for ((j=0;j<100;j++)); do
        #   file=`date | tr " " "-" | tr ":" "-"`
        #   ku top pod > ${dir}/${file}
        ...
        ./2020-06-09-main.just.ingress.svc.sh ${dir}
        sleep 30
        for ((j=0;j<100;j++)); do
           file=${j}
           ku top pod > ${dir}/${file}
